...
f = Expression(f"{MU_2}*exp(-((pow(x[0] - MU_4, 2) + pow(x[1] - MU_5, 2)) / MU_3))", degree=2, MU_2=MU_2, MU_3=MU_3, MU_4=MU_4, MU_5=MU_5)
...
# Plot sigma and u
plt.figure()
# plot(sigma, mode='color', vmin=-1, vmax=1, style=1)
name = f" for mu = [{MU_1}; {MU_2}; {1/MU_3}; {MU_4}; {MU_5}]"
c = plot(sigma, title = "Sigma" + name, scale= 20)
c.set_cmap ("CMRmap") 
# c.set_clim(0.0, 1.0)
plt.colorbar(c)
plt.figure()
p = plot(u, title = "u" + name)
p.set_cmap ("viridis")
# p.set_clim(, 2.0)
plt.colorbar(p)
# plot(u, mode='color', vmin=-1, vmax=1, style=1)
plt.show()
