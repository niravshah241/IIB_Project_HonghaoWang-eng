...
MU_1 = fem.Constant(msh, PETSc.ScalarType(5.))
MU_2 = fem.Constant(msh, PETSc.ScalarType(10.))
MU_3 = fem.Constant(msh, PETSc.ScalarType(1./0.02))
MU_4 = fem.Constant(msh, PETSc.ScalarType(0.5))
MU_5 = fem.Constant(msh, PETSc.ScalarType(0.5))

"""
Now the value can be updated inside without updating the function with the command:
MU_1.value = u[0], u is the 5 parameter
"""
...
f = MU_2.value * exp(-MU_3.value * ((x[0] - MU_4.value)**2 + (x[1] - MU_5.value)**2))
...
def f1(x):
    ...
    values[1, :] = np.sin(MU_1.value * x[0])
    return values


f_h1 = fem.Function(Q)

# GOes into the solve method
f_h1.interpolate(f1)
