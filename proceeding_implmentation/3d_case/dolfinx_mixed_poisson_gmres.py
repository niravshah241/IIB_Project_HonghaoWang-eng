...
marked_values = np.hstack([np.full_like(z_0_facets, 1), np.full_like(y_0_facets, 18), np.full_like(x_0_facets, 30)])
sorted_facets = np.argsort(marked_facets)
boundaries = dolfinx.mesh.meshtags(mesh, fdim, marked_facets[sorted_facets], marked_values[sorted_facets])
...
# dx = ufl.Measure("dx", domain=mesh, subdomain_data=mesh)
dx = ufl.dx
...
ksp.rtol = 1.e-8 # NOTE or ksp.setTolerances(1e-8) # rtol is the first argument of setTolerances
